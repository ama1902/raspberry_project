...
def mouse_click(event,x,y,flags,param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN: #even is clicking left mouse
        ...
        hueLow = max(0, hsv_value[0] - boundary)
        hueHigh = min(255, hsv_value[0] + boundary)
        satLow = max(0, hsv_value[1] - boundary)
        satHigh = min(255, hsv_value[1] + boundary)
        valLow = max(0, hsv_value[2] - boundary)
        valHigh = min(255, hsv_value[2] + boundary)
...
while True:
    ...
    if len(contours)>0: # find countors
        ...
        track.append((int(target_x), int(target_y)))
