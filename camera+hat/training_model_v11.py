#this version same as v9. but adds plot for pid and fixed step
...
with open('/home/ama/disertation_project/camera+hat/movement_log.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['timestamp', 'pan_angle', 'tilt_angle','target_x','target_y', 'movement_type'])  # Header

def log_movement(pan_angle, tilt_angle,target_x, target_y,movement_type):
    with open('/home/ama/disertation_project/camera+hat/movement_log.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([time.time(), pan_angle, tilt_angle,target_x, target_y,movement_type])
...
while True:
    ...
    if len(contours)>0: # find countors
        ...
        if training_method == STEADY:
            ...
            log_movement(current_pan_angle,current_tilt_angle, target_x,target_y, "STEADY")
        ...
        if training_method == PID:
            ...
            pantilthat.pan(pan_angle)
            pantilthat.tilt(tilt_angle)
            log_movement(pan_angle,tilt_angle, target_x,target_y, "PID")
